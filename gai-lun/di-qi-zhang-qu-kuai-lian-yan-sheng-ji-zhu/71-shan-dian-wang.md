...
（3a）子贡用自己的第二个公钥$$PublicKey_{A1}$$和卫国钱庄的$$PublicKey_{B1}$$产生第二个多重签名地址并依次构建承诺交易（Commitment Tx）和可撤销投递交易（Revocable Delivery Tx）。的输入是（2）中建立的筹资交易，输出有两个：①将0.1BTC打入地址，②将0.1BTC打入卫国钱庄地址。的输入是①，输出是将0.1BTC打入子贡地址。此后，子贡用签名和签名，并发给卫国钱庄；
