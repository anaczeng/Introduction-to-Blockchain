...
3.	在币基交易中，币基脚本就是那条在坊间广为流传的《泰晤士报》标题的加密值。因为没有可引用的上一个交易，所以币基交易中相当于输入秘钥签名的部分被替换成币基脚本，它可以是任意值。这条信息证明了创世块的产生不可能早于伦敦时间的2009年1月3日（对应北京时间的2009年1月4日）。此外，业内认为这也反映了中本聪这位创始者的动机。
...
存储前区块散列值构成哈希指针的一部分，以确保之前区块的变动一定引起后面区块的变动；散列值计算使用SHA-256哈希函数

32

Merkle树根

Merkle树根从包含在该区块的所有交易计算得出，以确保该区块内任何交易的变动必定引起该区块头部的变动；散列值计算使用SHA-256哈希函数

32

时间

使用Unix时间（详细介绍请参见本节的计算机小知识：Unix时间），为矿工开始计算该区块头散列值的时刻。不能小于前11个区块时间的中间值，且全节点不会接受头部大于全节点时钟未来两小时的区块

4 

nBits

经过编码的难度值，限定了该区块头的散列值不能大于该难度值。详细介绍请参见本节难度\(difficulty\)部分。

4

随机数（nonce）

矿工可以改变的随机数（每比特只有0或1两个取值），以使区块头的散列值小于nBits。如果32位的随机数全被测试完毕依然没有找到符合条件的散列值，那么矿工可以更新时间或修改币基合同（币基合同的改变会引起Merkle树根的改变）
...
用于私钥签名以证明比特币的归属；同时也是个脚本语言

不定

序列号

固定为0xffffffff，现阶段并未使用
...
TXID，每个交易都有的唯一标识符；散列值计算使用SHA-256哈希函数

32

索引（index）

用于指明交易中具体的输出，因为交易中可有多个输出

4

表4-5：外指每部分大小和功能的具体介绍。
...
花费比特币的数量，以“聪（satoshi）”为单位。输出的数量不能多于输入比特币的数量（币基交易除外）

8

公钥脚本字节数

公钥地址脚本的字节数。最多可有10,000字节
...
币基交易的输入：因为币基产生新的比特币，这些比特币并不是来自之前的交易，所以相比普通的交易，域稍有不同。
...
32字节的null，因为币基没有前外指（outpoint）

32

索引

0xffffffff，因为币基没有前外指（outpoint）
...
从创世块为区块0递增排序到特定区块的序号，字节数尽可能少

不定

币基脚本

任意不超过100字节的数据（减去区块高度的字节）。矿工一般在这里使用额外的随机数挖矿（币基交易的改变会引起Merkle树根的改变，从而可以重新计算散列值）
