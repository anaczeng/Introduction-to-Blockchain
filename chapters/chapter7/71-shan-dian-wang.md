...
（3a）子贡用自己的第二个公钥$$PublicKey_{A1}$$和卫国钱庄的$$PublicKey_{B1}$$产生第二个多重签名地址$$MultiSigAddress_2$$并依次构建承诺交易（Commitment Tx）$$C1_a$$和可撤销投递交易（Revocable Delivery Tx）$$RD1_a$$。的输入是（2）中建立的筹资交易，输出有两个：①将0.1BTC打入地址$$MultiSigAddress_2$$，②将0.1BTC打入卫国钱庄地址$$Address_B$$。$$RD1_a$$的输入是$$C1_a$$①，输出是将0.1BTC打入子贡地址$$Address_A$$。此后，子贡用$$PrivateKey_{A1}$$签名$$C1_a$$和$$PrivateKey_{A2}$$签名$$RD1_a$$，并发给卫国钱庄；

![](/assets/fig-7-2%283%29.png)

（3b）卫国钱庄操作流程和子贡一样，先获得多重签名地址$$MultiSigAddress_3$$，然后构建交易$$C1_b$$和$$RD1_b$$并签名发给子贡；

![](/assets/fig-7-4%285%29.png)

（4）子贡使用$$PrivateKey_{A1}$$和卫国钱庄使用$$PrivateKey_{B1}$$签名（2）中的筹资交易并将其广播。
...
（5）子贡用自己的第三个公钥$$PublicKey_{A3}$$和卫国钱庄的$$PublicKey_{B1}$$产生新的多重签名地址$$MultiSigAddress_4$$；

（6）子贡创建新的交易$$C2_a$$和$$RD2_a$$，并创建比$$RD1_a$$优先级更高的违约补偿交易（Breach Remedy Tx）$$BR1_a$$。子贡签名$$C2_a$$、$$RD2_a$$和$$BR1_a$$后发给卫国钱庄。

![](/assets/fig-7-7%288,9%29.png)

（7）卫国钱庄获得$$MultiSigAddress_5$$后，创建交易$$C2_b$$和$$RD2_b$$并签名后发给子贡；
...
开通微支付通道后的交易是撤销之前交易更新双方余额状态的过程。按照业务逻辑，之前的交易$$C1_a$$、$$RD1_a$$和$$C1_b$$、$$RD1_b$$都应作废，由新的交易$$C2_a$$、$$RD2_a$$和$$C2_b$$、$$$RD2_b$$取代；但是在技术层面，交易$$C1_a$$、$$RD1_a$$、$$C1_b$$、$$RD1_b$$和交易$$C2_a$$、$$RD2_a$$、$$C2_b$$、$$RD2_b$$是一样的。如何防止子贡后面反悔执行交易$$C1_a$$和$$RD1_a$$，这里引入交易$$BR1_a$$。假如子贡执行了交易$$C1_a$$和$$RD1_a$$，交易$$C1_a$$马上入块，但是交易$$RD1_a$$要在交易$$C1_a$$被1000个块确认后才能入块。在这大约10000分钟内，卫国钱庄发现了子贡的欺诈行为马上执行$$BR1_a$$。$$BR1_a$$是可以马上入块的，这时子贡就会失去所有资金。
...
（2）在HTLC中，由支付者子贡广播交易$$C2_b$$和由接收者卫国钱庄广播交易$$C1_a$$的后续实现是不一样的。下面就分别从这两方面来介绍HTLC合约的具体实现。
...
子贡广播交易$$C1_b$$后，如果卫国钱庄能在16小时内广播含有秘密R的交易$$HED1_b$$，他就能获取0.1BTC；如果卫国钱庄未能履约，子贡在16小时后可广播交易$$HT1_b$$，当交易$$HT1_b$$被1000个区块确认后，交易$$HTRD1_b$$被区块确认，子贡得到0.1BTC的资金返还。

由接收者广播承诺交易：

卫国钱庄广播交易$$C1_b$$后，如果他能在16小时内广播含有秘密R的交易$$HE1_a$$，在交易$$HE1_a$$被1000个区块确认后，交易$$HED1_a$$被区块确认，卫国钱庄获得0.1BTC。假如卫国钱庄未能履约，子贡在16小时候广播交易$$HTD1_a$$，她很快就获得0.1BTC的资金返还。
